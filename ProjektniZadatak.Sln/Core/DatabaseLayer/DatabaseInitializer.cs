        ...
        protected override void Seed(SavedTextDbContext context)
        {
            var savedTexts = new List<SavedText>()
            {
                new SavedText{ TextValue = @"Before the game begins, each player creates their player character and records the details (described below) on a character sheet. 
                                ...
                                During the game, players describe their PCs' intended actions, such as punching an opponent or picking a lock, and converse with the DM, 
                                who then describes the result or response", DateTimeOfText = DateTime.Now},
                new SavedText {TextValue = "Some random smaller text with some numbers 12312312 41234 12,12, 34", DateTimeOfText = DateTime.Now}
            };
